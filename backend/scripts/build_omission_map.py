...
def main(dry_run: bool) -> None:
    ...
    # script cannot silently restore what was excluded on purpose.
    artifacts: dict[str, list[int]] = {}
    withheld: dict[str, list[int]] = {}
    for statute in list(found):
        art = sorted(found[statute] & parser_artifacts(statute))
        if art:
            artifacts[statute] = art
        held = sorted(found[statute] & frozenset(held_records(statute)))
        if held:
            withheld[statute] = held
        found[statute] -= held_back(statute)

    print(f"statutes with declared omissions : {len(found)}")
    if artifacts:
        print()
        print("  PARSER ARTIFACTS — permanently excluded, not omissions at all:")
        for statute, ns in artifacts.items():
            print(f"    {statute}: {ns}  (schedule table rows misread)")
    if withheld:
        print()
        print("  HELD PENDING confirmation — claimed omitted by the bundled")
        print("  source, unconfirmed by any independent one:")
        for statute, ns in withheld.items():
            for n in ns:
                r = held_records(statute)[n]
                print(f"    {statute} s.{n}: {r.instrument} ({r.date}, "
                      f"{r.jurisdiction})")

    cited = sum(len(confirmed_records(s)) for s in found)
    print(f"  omissions traced to a named instrument : {cited}")
    print()
    ...
    statutes_out = {}
    for statute in sorted(found):
        cites = confirmed_records(statute)
        statutes_out[statute] = {"sections": {
            str(n): (cites[n].to_dict() if n in cites
                     else {"status": "omitted", "instrument": "", "date": "",
                           "jurisdiction": "", "note": ""})
            for n in sorted(found[statute])}}

    held_out = {s: {str(n): r.to_dict() for n, r in sorted(held_records(s).items())}
                for s in sorted(set(found) | set(withheld)) if held_records(s)}
    artifacts_out = {s: sorted(parser_artifacts(s))
                     for s in sorted(set(found) | set(artifacts))
                     if parser_artifacts(s)}
    pending = {k: sorted(v) for k, v in
               ((s, pending_verification(s)) for s in sorted(found)) if v}

    OUT.write_text(json.dumps({
        "_schema": "statute-omissions/2",
        "_note": ("Generated by scripts/build_omission_map.py from statute "
                  "source texts. Sections each Act declares omitted or "
                  "repealed, with the instrument where it has been traced. "
                  "LOWER BOUND — footnote-style omissions (PPC) are not "
                  "parsed. Do not hand-edit; re-run the script."),
        "_parser_artifacts": {
            "reason": ("Not omissions. Schedule table rows whose leading number "
                       "was read as a section declaration — e.g. CrPC s.14, "
                       "which is a live section. Permanently excluded."),
            "sections": artifacts_out,
        },
        "_held_pending_confirmation": {
            "reason": ("Claimed omitted by the bundled source, unconfirmed by "
                       "any independent source. A data-completeness gap, not a "
                       "jurisdiction ambiguity. Excluded, NOT reclassified as "
                       "in force. See docs/CITATION_VERIFICATION_SESSION_REPORT.md."),
            "sections": held_out,
        },
