...
def _run_fixture(fixture, config: OcrConfig, sample_interval: float) -> FixtureResult:
    ...
    if not payload.get("ok"):
        ...
        return FixtureResult(
            ...
            wall_clock_seconds=elapsed, pages=payload.get("pages"),
            memory=memory)

    hypothesis = payload.get("text") or ""
    reference = fixture.transcript_path.read_text(encoding="utf-8")
    pages = int(payload.get("pages") or 0) or None
    ...
    return FixtureResult(
        ...
        seconds_per_page=(elapsed / pages) if pages else None,
        pages=pages,
...
def _aggregate(results: list[FixtureResult]) -> dict:
    ...
    per_page = sorted(r.seconds_per_page for r in ok if r.seconds_per_page)
    ...
    return {
        ...
        "seconds_per_page_median": per_page[len(per_page) // 2] if per_page else None,
        "seconds_per_page_p95": _p95(per_page),
