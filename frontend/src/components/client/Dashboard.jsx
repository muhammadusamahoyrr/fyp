...
import { LangProvider, useLang, useIsMobile } from "@/lib/i18n.jsx";
...
import ModTools from "./ModTools.jsx";
import ModOverseas from "./ModOverseas.jsx";
...
/* ─── Helpers ───────────────────────────────────────────────── */
const getLiveDate = (lang = "en") =>
    new Date().toLocaleDateString(lang === "ur" ? "ur-PK" : "en-US", {
        weekday: "long", year: "numeric", month: "long", day: "numeric",
    });

const SIDE_ITEMS = [
    { id: "overview", icon: "home", label: "Overview", ur: "جائزہ" },
    ...
    { id: "overseas", icon: "search", label: "Overseas Desk", ur: "بیرونِ ملک ڈیسک" },
    ...
    { id: "tracking", icon: "clock", label: "Tracking", ur: "کیس ٹریکنگ" },
];
...
const DashboardInner = ({ go, isDark, toggleTheme, initialTab = "overview" }) => {
    ...
    const displayName = user?.full_name || "Client";
    ...
    // The mini-collapsed sidebar makes no sense inside the mobile drawer
    useEffect(() => {
        ...
        <ThemeCtx.Provider value={t}>
            <HeaderActionsCtx.Provider value={{ setHeaderActions }}>
                ...
                <div style={{
                    ...
                    <div style={{
                        width: isMobile ? 260 : (isCollapsed ? 70 : 238),
                        minWidth: isMobile ? 260 : (isCollapsed ? 70 : 238),
                        background: t.surface,
                        borderRight: `1.5px solid ${t.border}`,
                        display: "flex", flexDirection: "column",
                        transition: "all 0.4s cubic-bezier(0.4,0,0.2,1)",
                        boxShadow: isDark ? "none" : "3px 0 16px rgba(26,46,53,0.08)",
                        zIndex: isMobile ? 41 : 10,
                        overflow: "hidden",
                        ...(isMobile ? {
                            ...
                            {SIDE_ITEMS.map(item => {
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        {!isCollapsed && T(item.label, item.ur)}
                                        {!isCollapsed && (
                        ...
                        <div style={{
                            ...
                            <div style={{
                                ...
                                {!isCollapsed && (
                                    <div style={{ flex: 1, minWidth: 0 }}>
                                        <div style={{ fontSize: 14, fontWeight: 700, color: t.text, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{displayName}</div>
                                        <div style={{ fontSize: 12, color: t.textMuted }}>{T("Client", "کلائنٹ")}</div>
                                    </div>
                                )}
                                {!isCollapsed && (
                                    <button
                                        onClick={toggleLang}
                                        title={T("اردو میں دیکھیں", "Switch to English")}
                                        style={{
                            ...
                            {!isCollapsed && (
                                <div style={{ display: "flex", gap: 6, width: "100%" }}>
                                    ...
                                    >
                                        <Ic n="logout" s={14} /> {T("Sign Out", "سائن آؤٹ")}
                                    </button>
                                </div>
                            )}
                        </div>
                    ...
                    <div style={{ flex: 1, overflow: "hidden", display: "flex", flexDirection: "column" }}>
                        ...
                        {!noTopbar && (
                            <div style={{
                                ...
                                borderBottom: `1.5px solid ${t.border}`,
                                display: "flex", alignItems: "center", justifyContent: "space-between",
                                background: isDark ? "rgba(26,46,53,0.82)" : "rgba(255,255,255,0.82)",
                                backdropFilter: "blur(18px) saturate(180%)",
                                WebkitBackdropFilter: "blur(18px) saturate(180%)",
                                position: "sticky", top: 0, zIndex: 20,
                                transition: "all 0.4s",
                                boxShadow: isDark ? "0 1px 0 rgba(64,240,220,0.06)" : "0 2px 12px rgba(26,46,53,0.08)",
                            }}>
                                <div style={{ display: "flex", alignItems: "center", gap: 10, minWidth: 0 }}>
                                    ...
                                    <div style={{ minWidth: 0 }}>
                                        <h1 style={{
                                            ...
                                            {(i => i ? T(i.label, i.ur) : "")(SIDE_ITEMS.find(i => i.id === tab))}
                                        </h1>
                                        <p style={{ fontSize: 12, color: t.textMuted, marginTop: 2, whiteSpace: "nowrap" }}>{liveDate}</p>
                                    </div>
                                </div>
                        ...
                        {/* Floating menu button for full-bleed tabs (no topbar) on mobile */}
                        {isMobile && noTopbar && !mobileNavOpen && (
                            <button
                                onClick={() => setMobileNavOpen(true)}
                                aria-label="Open menu"
                                style={{
                        ...
                        <div
                            ...
                            style={{
                                flex: 1,
                                padding: noPadding ? 0 : (isMobile ? 14 : 32),
